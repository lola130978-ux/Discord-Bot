...
async function checkPlayer(tracked) {
    const contribution =
        await getPlayerLeague(tracked.robloxUserId);
...
function parseWithdrawal(text) {
    ...
    // Gems
    const gemMatch = value.match(
        /^([\d,.]+)\s*(k|m|b|t)?\s*(?:gems?|diamonds?)$/
    );
    ...
    // Random Huge
    const hugeMatch = value.match(
        /^(\d+)\s+(?:random\s+)?huge(?:s?)$/
    );

    if (hugeMatch) {
        const amount = Number(hugeMatch[1]);
    ...
    // Titanic
    const titanicMatch = value.match(
        /^(\d+)\s+titanic(?:s?)$/
    );

    if (titanicMatch) {
        const amount = Number(titanicMatch[1]);
...
// ============================================================
// CHECK WITHDRAWAL CAN BE PAID
// ============================================================

function canWithdraw(bank, withdrawal) {
    if (withdrawal.type === "gems") {
        return bank.gems >= withdrawal.amount;
    }

    if (withdrawal.type === "huge") {
        const hugeItem =
            bank.items.find(
                item =>
        ...
        return (
            hugeItem &&
            hugeItem.amount >= withdrawal.amount
        );
    }

    if (withdrawal.type === "titanic") {
        return (
...
function removeWithdrawalFromBank(
    ...
    if (!canWithdraw(bank, withdrawal)) {
        return false;
    }

    if (withdrawal.type === "gems") {
        bank.gems -= withdrawal.amount;
        return true;
    }
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    return interaction.reply({
                        content:
                            "❌ You don't have enough of that reward in your personal bank.",
                        ephemeral: true
                    });
                }
                ...
                await interaction.reply({
                    content:
                        ...
                        "Your reward will only be removed if the request is approved.",
                    ephemeral: true
                });
            ...
            if (
                ...
                if (
                    ...
                    request.failureReason =
                        "The user no longer has enough of the requested reward.";

                    saveData();

                    return interaction.reply({
                        content:
                            "❌ The user no longer has enough of the requested reward. Nothing was removed.",
                        ephemeral: true
                    });
                }

                // Actually remove the reward.
                const removed =
                ...
                if (!removed) {
                    return interaction.reply({
                        content:
                            "❌ The withdrawal could not be completed. Nothing was removed.",
                        ephemeral: true
                    });
                }
